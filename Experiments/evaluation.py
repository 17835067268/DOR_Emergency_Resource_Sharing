...
dir_all = df.iloc[:, 2:].to_numpy().reshape((-1, N, R)) #所有的不确定消耗样本
...
def compute_resource_consumption_t0(hir_t0, gir_numpy_t0, x_iir_t0, dir_all, scenario_t0=1, N_s=10):
    shared_in_t0 = x_iir_t0.sum(axis=1).T  # shape (18, 6)
    shared_out_t0 = x_iir_t0.sum(axis=2).T
    left_sum_t0 = hir_t0 + gir_numpy_t0 + shared_in_t0 - shared_out_t0
    sample_id = 0
    index_t0 = (scenario_t0 - 1) * N_s + sample_id  # 初始情景对应样本数据的第一个样本索引。即t0时刻某情景下发生的真实消耗数据以各情景下第一个样本数据代表。若scenario_t0 = 1，则index_t0 =0，若scenario_t0 = 2，则index_t0 =2...
    dir_ns = dir_all[index_t0]  # shape (18, 6)
    bir_t0 = np.minimum(left_sum_t0, dir_ns)  # 消耗不能超过当前持有
    bir_t0  = np.maximum(bir_t0 , 0) #控制bir≥0
    # 初始化新的资源持有矩阵
...
bir_t0, hir_ns_t1 = compute_resource_consumption_t0(hir_t0, gir_numpy_t0, x_iir_t0, dir_all, scenario_t0=1, N_s=10) # shape (18,6)
...
def define_cvxpy_variables() -> tuple[list[cp.Variable], list[cp.Variable], cp.Variable, cp.Variable]:
    ...
    fai_t1 = [cp.Variable((N, N), boolean=True, name=f"fai_r{r}") for r in range(R)]  #16(k)
...
def build_total_transport_cost(fai_t1, distance_matrix: np.ndarray) -> cp.Expression:
    ...
    cost_terms = []
    for r in range(R):
        row_terms = [cp.sum(cp.multiply(fai_t1[r][i, :], distance_matrix[i, :])) for i in range(N)]
...
def build_constraints(
    ...
    M = 10000
    for r in range(R):
        constraints += [x_iir_t1[r] <= M * fai_t1[r]] 
        constraints += [x_iir_t1[r] >= epsilon * fai_t1[r]]
        constraints += [x_iir_t1[r] >= 0, x_iir_t1[r] <= 1]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            rhs = -pai_t1[s - 1, sid] + cp.sum(cp.multiply(k_ir * w_s, -hir_ns_t1 - gir_numpy_t1 - shared_in)) + cp.sum(total_transport_cost)
            constraints += [lhs >= rhs]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            rhs  = -pai_t1[s - 1, sid] + expr + cp.sum(total_transport_cost)
            constraints += [rhs <= 0]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            sid = sample_id_array[idx]
            constraints += [-pai_t1[s - 1, sid] + cp.sum(total_transport_cost) <= 0]
    ...
    for r in range(R):
        for i in range(N):
            out_ = cp.sum(x_iir_t1[r][i, :]) - x_iir_t1[r][i, i]
            in_  = cp.sum(x_iir_t1[r][:, i]) - x_iir_t1[r][i, i]
            constraints += [
...
# 参数设定
out_of_sample_path = "/Sample_data/simulated_demand_structured_Ns_20.csv"
out_of_sample = pd.read_csv(out_of_sample_path)
out_of_sample_all = out_of_sample.iloc[:, 2:].to_numpy().reshape((-1, N, R))
N_post = 20 #后验样本out_of_sample的数量 
hir_t1 = pd.read_csv("hir_t1.csv", index_col=0).to_numpy()

shared_in_t1 = pd.read_csv("result/number_share_in_t1.csv", index_col=0).to_numpy()
shared_out_t1 = pd.read_csv("result/number_share_out_t1.csv", index_col=0).to_numpy()
...
for scenario_t1 in range(1, 15):  # 1 到 14
    sample_id = 0 #0-19之间取10个样本。sample_id = 0 表示第1个样本；sample_id = 1 表示第2个样本
    index_t1 = (scenario_t1 - 1) * N_post  + sample_id
    dir_ns_post = out_of_sample_all[index_t1]
    available_no_sharing = hir_t1 + gir_numpy_t1
    left_sum_t1 = hir_t1 + gir_numpy_t1 + shared_in_t1 - shared_out_t1 
    bir_t1 = np.minimum(left_sum_t1, dir_ns_post)
    bir_t1 = np.maximum(bir_t1, 0)


    satisfaction_rate = np.divide(left_sum_t1, dir_ns_post, out=np.ones_like(dir_ns_post, dtype=float), where=dir_ns_post > 0)
    ...
    redundancy_rate = np.divide(np.maximum(left_sum_t1 - dir_ns_post, 0), dir_ns_post, out=np.zeros_like(dir_ns_post, dtype=float), where=dir_ns_post > 0)
    ...
    shortage_matrix = np.maximum(dir_ns_post - left_sum_t1, 0)
    ...
    for i in range(N):
        ...
        satisfaction_rate_row = {"scenario_t1": scenario_t1, "主体": subject_name}
        satisfaction_rate_no_sharing_row = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_rate_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_rate_no_sharing_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
        redundancy_rate_row   = {"scenario_t1": scenario_t1, "主体": subject_name}
        redundancy_rate_no_sharing_row   = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_matrix_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_matrix_no_sharing_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_cost_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
        shortage_cost_no_sharing_row     = {"scenario_t1": scenario_t1, "主体": subject_name}
...
# 保存为 CSV 文件
df_satisfaction_rate.to_csv("缩放1_satisfaction_rate_matrix.csv", index=False)
df_satisfaction_rate_no_sharing.to_csv("缩放1_satisfaction_rate_no_sharing_matrix.csv", index=False)
df_redundancy_rate.to_csv("缩放1_redundancy_rate_matrix.csv", index=False) 
df_redundancy_rate_no_sharing.to_csv("缩放1_redundancy_rate_no_sharing_matrix.csv", index=False) 
df_shortage_rate.to_csv("缩放1_shortage_rate_matrix.csv", index=False)
df_shortage_rate_no_sharing.to_csv("缩放1_shortage_rate_no_sharing_matrix.csv", index=False)
df_shortage_matrix.to_csv("缩放1_shortage_matrix.csv", index=False)
df_shortage_matrix_no_sharing.to_csv("缩放1_shortage_no_sharing_matrix.csv", index=False)
df_shortage_cost.to_csv("缩放1_shortage_cost_matrix.csv", index=False)
df_shortage_cost_no_sharing.to_csv("缩放1_shortage_cost_no_sharing_matrix.csv", index=False)
df_total_shortage_cost_per_subject.to_csv("缩放1_total_cost.csv", index=False)
df_total_shortage_cost_no_sharing_per_subject.to_csv("缩放1_total_cost_no_sharing.csv", index=False)
