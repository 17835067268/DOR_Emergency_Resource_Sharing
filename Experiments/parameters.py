##############################xiir换成比例#########################################
"""
lamda =  [500, 1000, 1500, 2000, 2500, 3000]
...
def save_bir_and_hir_to_csv(bir_t0, hir_ns_t1, N, R):
    """
    将 bir_t0 和 hir_ns_t1 保存为 CSV 文件，命名为 bir_t0.csv 和 hir_t1.csv。
    每行对应一个主体，每列对应一个资源，列名为 P01_R1 ~ P18_R6。
    """
...
def build_constraints(
    ...
    for s in range(1, S + 1):
        ...
        for idx in np.where(scenario_array == s)[0]:
            ...
            accum += (prob / Ns[s - 1]) * (q_mat + theta_mat)
