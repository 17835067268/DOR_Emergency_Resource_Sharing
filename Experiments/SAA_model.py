...
SAA_csv_path = "/Sample_data/simulated_demand_structured_Ns_5.csv"
SAA_df = pd.read_csv(SAA_csv_path)
...
SAA_dir_all = SAA_df.iloc[:, 2:].to_numpy().reshape((-1, N, R)) 

# ------------------ t=0资源状态计算：bir_t0, hir_t1 ------------------ #
def compute_resource_consumption_t0(hir_t0, gir_numpy_t0, x_iir_t0, dir_all, scenario_t0=1, N_s=2):
...
bir_t0, hir_ns_t1 = compute_resource_consumption_t0(hir_t0, gir_numpy_t0, x_iir_t0, dir_all, scenario_t0=1, N_s=2) # shape (18,6)


available_sharing_t1 = gir_numpy_t1 + hir_ns_t1
scenario_array = SAA_df['scenario'].to_numpy()
sample_id_array = SAA_df['sample_id'].to_numpy() 
bir_t0, hir_ns_t1 = compute_resource_consumption_t0(hir_t0, gir_numpy_t0, x_iir_t0, dir_all)


# ------------------ 构造qir_ns_t1：样本下的对外真实需求 ------------------ #
def build_qir_ns_t1(S, scenario_array, sample_id_array, SAA_dir_all, hir_ns_t1, gir_numpy_t1):
    SAA_qir_ns_t1 = {}
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            SAA_dir_ns = SAA_dir_all[idx]  # shape: (N, R)
            SAA_qir_matrix = np.maximum(SAA_dir_ns - hir_ns_t1 - gir_numpy_t1, 0)
            SAA_qir_ns_t1[s][sample_id] = SAA_qir_matrix
    return SAA_qir_ns_t1

SAA_qir_ns_t1 = build_qir_ns_t1(S, scenario_array, sample_id_array, SAA_dir_all, hir_ns_t1, gir_numpy_t1)
...
def build_constraints_saa(
    x_iir_t1, fai_t1,  # 决策变量
    hir_ns_t1, gir_numpy_t1, shared_in, shared_out, 
    SAA_qir_ns_t1
):
    constraints = []
    ...
    for r in range(R):
        for i in range(N):
            ...
            constraints += [
                out_ <= 10000 * (1 - l_ir[r][i]),
                in_  <= 10000 * l_ir[r][i]
            ]

    # Constraint 4: 
    q_mean = np.mean([SAA_qir_ns_t1[scenario_array[idx]][sample_id_array[idx]]for idx in range(M_samples)], axis=0)  # shape (N, R)
    for i in range(N):
        for r in range(R):
            constraints += [
                q_mean[i, r] -  cp.sum(x_iir_t1[r][:, i]) >= 0
            ]
    return constraints

# ====================== 目标函数 ====================== 
def build_objective_saa(
    x_iir_t1, fai_t1,
    k_ir, w_matrix, 
    shared_in, SAA_qir_ns_t1, SAA_dir_all,
    scenario_array, SAA_df,
    distance_matrix,lamda=1600
):
    R, N = len(x_iir_t1), x_iir_t1[0].shape[0]
    M_samples = len(SAA_df)
    ...
    for idx in range(M_samples):
        s = scenario_array[idx]
        m = SAA_df[idx]
        q_mat = SAA_qir_ns_t1[s][m]   # shape (N, R)
        w_mat = w_matrix[s - 1]       # shape (N, R)
        shortage = cp.pos(q_mat - shared_in)  # (N, R)
        weighted_shortage = cp.multiply(k_ir * w_mat, shortage)  # (N, R)
        total_shortage_cost += cp.sum(weighted_shortage)
    transport_cost = build_total_transport_cost(fai_t1, distance_matrix)

    total_cost_expr = (total_shortage_cost  + transport_cost)/ M_samples

    return cp.Minimize(total_cost_expr), total_cost_expr



# ====================== 模型求解函数 ====================== #
def solve_saa_ratio_model(
    k_ir, w_matrix,  # 权重
    hir_ns_t1, gir_numpy_t1, available_sharing_t1, 
    SAA_qir_ns_t1, SAA_dir_all, scenario_array, sample_id_array, 
    distance_matrix, lamda=1600
):

    x_iir_t1, fai_t1 = define_cvxpy_variables()

    shared_in, shared_out = build_shared_matrices(x_iir_t1, available_sharing_t1)

    objective, total_cost_expr = build_objective_saa(
        x_iir_t1, fai_t1,
        k_ir, w_matrix,
        shared_in, SAA_qir_ns_t1, SAA_dir_all,
        scenario_array, sample_id_array,
        distance_matrix,
        lamda=lamda
    )

    constraints = build_constraints_saa(
            x_iir_t1, fai_t1,  
            hir_ns_t1, gir_numpy_t1, shared_in, shared_out,
            SAA_qir_ns_t1
    )
...
# ------------------ 输出一些内容 ------------------ #
result_saa = solve_saa_ratio_model(
    k_ir, w_matrix,
    hir_ns_t1, gir_numpy_t1, available_sharing_t1,
    SAA_qir_ns_t1, SAA_dir_all, scenario_array, sample_id_array,
    distance_df_numpy
...
print("Objective value:", result_saa["problem"].value)
print("Objective status:", result_saa["problem"].status)

overall_x_iir_t1 = []
for r in range(R):
    x_r = result_saa["x_iir_t1"][r].value  # shape: (N, N)
    for i in range(N):
        overall_x_iir_t1.append([r] + x_r[i].tolist())  
columns = ["r"] + [f"P{j}" for j in range(N)]
df = pd.DataFrame(overall_x_iir_t1, columns=columns)
df.to_csv("SAA(5)_x_iir_t1_stacked_with_r.csv", index=False)
