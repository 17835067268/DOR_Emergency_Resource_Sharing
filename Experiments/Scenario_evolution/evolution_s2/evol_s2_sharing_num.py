...
# ------------------ 样本数据载入(样本内) ------------------ #
csv_path = "evolution_sample/s2_simulated_demand_structured_Ns_40.csv"
...
level_table = {
    ...
    5:  {1: "一般", 2: "较低", 3: "较低", "T1": "较低", "T2": "较低", "D": "较低"},
    ...
    11: {1: "较高", 2: "一般", 3: "一般", "T1": "较低", "T2": "较高", "D": "较高"},
...
# gir_t3, gir_numpy_t3 = generate_external_support(list(map(str, range(N))), role_groups, external_assistance_rules_t3)



scenario_t1 = 2
def generate_external_support_t2_based_on_t1(
    scenario_t1, subject_info, infected_communities, resource_set, external_assistance_rules_t2
):
    infected_comms = infected_communities.get(scenario_t1, set())

    valid_subjects = set()

    # 收集所有感染社区内的主体
    for idx, (comm, town, dist) in subject_info.items():
        if comm in infected_comms:
            valid_subjects.add(str(idx))

    # 收集这些社区对应的街道主体
    town_set = set()
    for comm in infected_comms:
        for town, comms in town_to_communities.items():
            if comm in comms:
                town_set.add(town)

    for idx, (comm, town, dist) in subject_info.items():
        if town in town_set and comm is None:  # 街道级主体
            valid_subjects.add(str(idx))

    # 添加区级主体
    valid_subjects.add("17")

    filtered_role_groups = {
        "neighbourhood_committee": [i for i in neighbourhood_committee if str(i) in valid_subjects],
        "property_committee": [i for i in property_committee if str(i) in valid_subjects],
        "town_subject": [i for i in town_subject if str(i) in valid_subjects],
        "district_subject": [i for i in district_subject if str(i) in valid_subjects],
    }
    filtered_role_groups = {
        "neighbourhood_committee": [i for i in neighbourhood_committee if i in valid_subjects],
        "property_committee": [i for i in property_committee if i in valid_subjects],
        "town_subject": [i for i in town_subject if i in valid_subjects],
        "district_subject": [i for i in district_subject if i in valid_subjects],
    }

    gir_dict, gir_numpy = generate_external_support(
        subject_ids=list(map(str, range(len(subject_info)))),
        role_groups=filtered_role_groups,
        rules=external_assistance_rules_t2
    )
    return gir_dict, gir_numpy


# 前后两天的gir_t
gir_t3, gir_numpy_t3 = generate_external_support(list(map(str, range(N))), role_groups, external_assistance_rules_t3)
gir_t2, gir_numpy_t2 = generate_external_support_t2_based_on_t1(
    scenario_t1 = scenario_t1,
    subject_info=subject_info,
    infected_communities=infected_communities,
    resource_set=resource_set,

    ##(仅前一天疫区主体r1-r6均匀分摊180单位)
    external_assistance_rules_t2={
        "neighbourhood_committee": {"医用口罩": 10, "防护服": 10, "消毒液": 10, "护目镜": 10, "帐篷": 0},
        "property_committee": {"医用口罩": 10, "防护服": 10, "消毒液": 10, "护目镜": 10},
        "town_subject": {"医用口罩": 12, "防护服": 12, "消毒液": 10, "护目镜": 12, "社区卫生服务中心医护人员": 2},
        "district_subject": {"医用口罩": 13, "防护服": 13, "护目镜": 26}
    }
)


available_sharing_t2 = gir_numpy_t2 + hir_t2 
df_x_iir = pd.read_csv("result/s2_x_iir_t2_stacked_with_r.csv")
data_remove_first_column = df_x_iir.iloc[:, 1:].to_numpy()  
x_iir_t2 = data_remove_first_column.reshape(R, N, N)
dir_all = df.iloc[:, 2:].to_numpy().reshape((-1, N, R))
 
# ------------------ 前一天的共享情况(t2) ------------------ #
def shared_matrices_t2(x_iir_t2, available_sharing_t2):
    """返回 (N,R) 的 shared_in / shared_out 绝对数量(NumPy实现)"""
    abs_mats = [] 
    shared_in_t2 = []
    shared_out_t2 = []
    for r in range(R):
        abs_mat = x_iir_t2[r] * available_sharing_t2[:, r][:, None] 
        abs_mats.append(abs_mat)  
        shared_in_t2.append(np.sum(abs_mat, axis=0))   
        shared_out_t2.append(np.sum(abs_mat, axis=1))  
    shared_in_t2= np.vstack(shared_in_t2).T   # shape: (N, R)
    shared_out_t2 = np.vstack(shared_out_t2).T # shape: (N, R)
    return abs_mats, shared_in_t2, shared_out_t2 

abs_mats, shared_in_t2, shared_out_t2 = shared_matrices_t2(x_iir_t2, available_sharing_t2)
abs_mats = np.rint(abs_mats).astype(int) 
shared_in_t2 = np.rint(shared_in_t2).astype(int) 
shared_out_t2 = np.rint(shared_out_t2).astype(int)
scenario_array = df['scenario'].to_numpy()
sample_id_array = df['sample_id'].to_numpy()


def compute_resource_consumption_t1(hir_t2, gir_numpy_t2, x_iir_t2, dir_all, scenario_t2=2, N_s=40):
    shared_in_t2 = x_iir_t2.sum(axis=1).T  # shape (18, 6)
    shared_out_t2 = x_iir_t2.sum(axis=2).T
    left_sum_t2 = hir_t2 + gir_numpy_t2 + shared_in_t2 - shared_out_t2
    sample_id = 0
    index_t2 = (scenario_t2 - 1) * N_s + sample_id  
    dir_ns = dir_all[index_t2]  # shape (18, 6)  
    bir_t2 = np.minimum(left_sum_t2, dir_ns) 
    bir_t2  = np.maximum(bir_t2, 0) 
    hir_ns_t3 = np.zeros_like(hir_t2)
    ...
    if consumable_indices:
        hir_ns_t3[:, consumable_indices] = np.maximum(
            hir_t2[:, consumable_indices]
            + gir_numpy_t2[:, consumable_indices]
            + shared_in_t2[:, consumable_indices]
            - shared_out_t2[:, consumable_indices]
            - bir_t2[:, consumable_indices], 0
        )  
    if reusable_indices:
        hir_ns_t3[:, reusable_indices] = np.maximum(
            hir_t2[:, reusable_indices]
            + gir_numpy_t2[:, reusable_indices]
            + shared_in_t2[:, reusable_indices]
            - shared_out_t2[:, reusable_indices], 0
        ) 
    return bir_t2, hir_ns_t3
bir_t2, hir_ns_t3 = compute_resource_consumption_t1(hir_t2, gir_numpy_t2, x_iir_t2, dir_all, scenario_t2=2, N_s=40) 

available_sharing_t3 = gir_numpy_t3 + hir_ns_t3 


# ------------------ 构造qir_ns_t3：样本下的对外真实需求 ------------------ #
def build_qir_ns_t3(S, scenario_array, sample_id_array, dir_all, hir_ns_t3, gir_numpy_t3):
    qir_ns_t3 = {}
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            dir_ns = dir_all[idx]
            qir_matrix = np.maximum(dir_ns - hir_ns_t3 - gir_numpy_t3, 0)
            qir_ns_t3[s][sample_id] = qir_matrix
    return qir_ns_t3 

qir_ns_t3 = build_qir_ns_t3(S, scenario_array, sample_id_array, dir_all, hir_ns_t3, gir_numpy_t3)



# ------------------ 情景概率载入函数(这块载入的是已知t2时刻情景实现下，t3的概率转移矩阵) ------------------ #
def get_scenario_prob(day, csv_path="evolution_sample/scenario_probability_Marcov_t1_s2.csv"):
...
# ------------------ 模糊集半径 theta_s ------------------ #
theta_s = np.array([1.76, 1.84, 1.82, 1.88, 1.95, 1.93, 1.93, 1.94, 1.93, 1.94, 1.94, 1.96, 1.93, 1.97])
...
def define_cvxpy_variables() -> tuple[list[cp.Variable], list[cp.Variable], cp.Variable, cp.Variable]:
    ...
    x_iir_t3 = [cp.Variable((N, N), nonneg=True, name=f"x_r{r}") for r in range(R)]
    # 替换为线性化的形式：Big-M = 1
    fai_t3 = [cp.Variable((N, N), boolean=True, name=f"fai_r{r}") for r in range(R)]
    pai_t3 = cp.Variable((S, N_s), name="pai_t3")
    gamma_t3 = cp.Variable(S, name="gamma_t3")
    return x_iir_t3, fai_t3, pai_t3, gamma_t3
...
def build_shared_matrices(x_iir_t3: list[cp.Expression], available_sharing_t3: np.ndarray) -> tuple[cp.Expression, cp.Expression]:
    """返回 (N,R) 的 shared_in / shared_out 绝对数量表达式"""
    shared_in, shared_out = [], []
    for r in range(R):
        abs_mat = cp.multiply(x_iir_t3[r], available_sharing_t3[:, r][:, None])  # (N,N)
        shared_in.append(cp.sum(abs_mat, axis=0))   
        shared_out.append(cp.sum(abs_mat, axis=1))  
    return cp.vstack(shared_in).T, cp.vstack(shared_out).T 



# ------------------ 构造距离矩阵乘以是否共享的总运输距离 ------------------ #
def build_total_transport_cost(fai_t3, distance_matrix: np.ndarray) -> cp.Expression:
    N = distance_matrix.shape[0]
    cost_terms = []
    for r in range(R):
        row_terms = [cp.sum(cp.multiply(fai_t3[r][i, :], distance_matrix[i, :])) for i in range(N)]
...
def build_constraints(
    x_iir_t3, fai_t3, pai_t3, gamma_t3,
    k_ir, w_matrix, w_flat,
    shared_in, shared_out,
    dir_all, hir_ns_t3, gir_numpy_t3,
    ...
    constraints = []
    S = len(theta_s)
    R, N = len(x_iir_t3), x_iir_t3[0].shape[0]

    # Constraint 1: 
    epsilon = 1e-5
    M = 10000
    for r in range(R):
        constraints += [x_iir_t3[r] <= M * fai_t3[r]] 
        constraints += [x_iir_t3[r] >= epsilon * fai_t3[r]]
        constraints += [x_iir_t3[r] >= 0, x_iir_t3[r] <= 1]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            rhs = -pai_t3[s - 1, sid] + cp.sum(cp.multiply(k_ir * w_s, -hir_ns_t3 - gir_numpy_t3 - shared_in)) + cp.sum(total_transport_cost)
            constraints += [lhs >= rhs]

    # Constraint 3:
    gamma_lb = [float(np.linalg.norm(k_ir.flatten() * w_flat[s - 1], 2)) for s in range(1, S + 1)]
    for s in range(1, S + 1):
        constraints += [gamma_t3[s - 1] >= gamma_lb[s - 1]]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            ...
            rhs  = -pai_t3[s - 1, sid] + expr + cp.sum(total_transport_cost)
            constraints += [rhs <= 0]
    ...
    for s in range(1, S + 1):
        ...
        for idx in indices:
            sid = sample_id_array[idx]
            constraints += [-pai_t3[s - 1, sid] + cp.sum(total_transport_cost) <= 0]
    ...
    for s in range(1, S + 1):
        ...
        for idx in np.where(scenario_array == s)[0]:
            q_mat = qir_ns_t3[s][sample_id_array[idx]]
            q_sum += q_mat
            count += 1
        q_mean = q_sum / count
    ...
    # # Constraint 7: 
    constraints += [hir_ns_t3 + gir_numpy_t3 - shared_out >= 0]

    
    # # Constraint 8: 
    for r in range(R):
        constraints += [cp.diag(x_iir_t3[r]) == 0]
    ...
    for r in range(R):
        for i in range(N):
            out_ = cp.sum(x_iir_t3[r][i, :]) - x_iir_t3[r][i, i]
            in_  = cp.sum(x_iir_t3[r][:, i]) - x_iir_t3[r][i, i]
            constraints += [
...
# ====================== 目标函数 ====================== 
def build_objective(pai_t3, gamma_t3, scenario_prob, theta_s):
    weights_sample = np.repeat(scenario_prob / N_s, N_s)  # len = S*N_s
    objective_section1 = cp.sum(cp.multiply(cp.vec(pai_t3), weights_sample))
    objective_section2 = cp.sum(cp.multiply(gamma_t3, scenario_prob * theta_s))
    return cp.Minimize(objective_section1 + objective_section2), objective_section1, objective_section2

# ====================== 模型求解函数 ====================== #
def solve_dro_ratio_model(
    k_ir, w_matrix, w_flat,
    hir_ns_t3, gir_numpy_t3, available_sharing_t3,
    qir_ns_t3, theta_s, scenario_prob_t,
    distance_matrix
):
    
    x_iir_t3, fai_t3, pai_t3, gamma_t3 = define_cvxpy_variables()

    shared_in, shared_out = build_shared_matrices(x_iir_t3, available_sharing_t3)

    transport_cost = build_total_transport_cost(fai_t3, distance_matrix)
    ...
    objective,objective_section1,objective_section2 = build_objective(pai_t3, gamma_t3, scenario_prob_t, theta_s)
...
subject_names = [f"主体{i+1}" for i in range(18)]
number_share_in_t2 = pd.DataFrame(shared_in_t2, columns=resource_names, index=subject_names)
number_share_out_t2 = pd.DataFrame(shared_out_t2, columns=resource_names, index=subject_names)
number_share_in_t2.to_csv("s2_number_share_in_t2.csv",index=True) 
number_share_out_t2.to_csv("s2_number_share_out_t2.csv",index=True) 


abs_mat_blocks = np.vstack(abs_mats)  
index_names = [f"R{r+1}_主体{i+1}" for r in range(R) for i in range(18)]
column_names = [f"主体{j+1}" for j in range(18)]
df_abs_mat_all = pd.DataFrame(abs_mat_blocks, index=index_names, columns=column_names)
df_abs_mat_all.to_csv("s2_abs_shared_matrix_all_t2.csv", encoding="utf-8-sig") 