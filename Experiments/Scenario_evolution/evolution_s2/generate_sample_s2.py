...
def extract_region(subject_name):
    for level_name in ["社区", "街道", "区"]:  
        if level_name in subject_name:  	
            ...
            return prefix, level_name #返回 ("a1", "社区")
    return None, None
...
# ========= 资源调节系数，资源1-6依次为["医用口罩","防护服","消毒液","护目镜","帐篷","社区卫生服务中心医护人员"] ==========
alpha = {1: 1, 2: 1, 3: 0.5, 4: 1, 5: 0.4, 6: 0.1} #6种资源的需求均值调节系数
beta = {1: 1.1, 2: 1, 3: 0.5, 4: 1, 5: 0, 6: 0} #6种资源的需求标准差调节系数

# ====================== “很低”需求等级下的base_mu 和 base_sigma ======================
base_mu = np.array([
    [12, 4, 5, 4, 1, 0],  #N95口罩和防护服为1:3，一天每人要换3个。防护服=护目镜.一个社区一个核酸点，需要一顶帐篷。
    [18, 6, 20, 6, 0, 0], #感染社区物业消杀工作需要的资源会比社区居委会多一些。
    [12, 4, 5, 4, 1, 0],  
    [18, 6, 20, 6, 0, 0], 
    [12, 4, 5, 4, 1, 0],  
    [18, 6, 20, 6, 0, 0],
    ...
    [18, 6, 20, 6, 0, 0],
    [12, 4, 5, 4, 1, 0],   
    [18, 6, 20, 6, 0, 0], #前面所有社区的居委会和物业在base需求等级下对6种资源的需求量均一致
    [24, 8, 0, 8, 0, 8], #一个社区一个核酸检测点，一个核酸检测点2个医护人员。另外流行病学调查和转运分别需要1个医护人员
    [18, 6, 0, 6, 0, 6],
    [18, 6, 0, 6, 0, 6],
    [24, 8, 0, 8, 0, 0] #疾控流调任务，流调时，会派出5-10名流调人员。此处设置为8
])  #18个主体对6种资源在base需求等级下的需求均值
...
##3σ法则：对于正态分布，99.7%的数据落在 [μ-3σ, μ+3σ] 范围内。3σ就是最大震荡范围max_fluctuations
base_sigma = np.array(max_fluctuations/3) #18个主体对6种资源在base需求等级下的需求标准差
...
csv_path = "/evolution_sample/s2_simulated_demand_structured_Ns_30.csv" 
