...
overall_x_iir_t2 = []
for r in range(R):
    x_r = result["x_iir_t2"][r].value  
    for i in range(N):
        overall_x_iir_t2.append([r] + x_r[i].tolist())
columns = ["r"] + [f"P{j}" for j in range(N)]
df = pd.DataFrame(overall_x_iir_t2, columns=columns)
df.to_csv("s13_x_iir_t2_stacked_with_r.csv", index=False)
